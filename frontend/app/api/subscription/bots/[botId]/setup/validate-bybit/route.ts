...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const response = await fetchBackend(`/subscriptions/${encodeURIComponent(botId)}/setup/validate-bybit`, {
      method: "POST",
    ...
    if (!response.ok) {
      const detail = String(payload.detail ?? payload.error ?? payload.message ?? "Bybit API validation failed");
      return NextResponse.json({ error: detail, detail }, { status: response.status });
    }

    return NextResponse.json(payload, { status: response.status });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to validate Bybit API";
