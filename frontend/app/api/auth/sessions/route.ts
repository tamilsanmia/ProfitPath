...
export async function GET() {
  ...
  try {
    const response = await fetchBackend(
      `/auth/sessions?email=${encodeURIComponent(email)}&session_token=${encodeURIComponent(token)}`,
      { cache: "no-store" }
    );
    ...
    return NextResponse.json({ sessions: Array.isArray(payload?.sessions) ? payload.sessions : [] }, { status: 200 });
  } catch {
    return NextResponse.json({ sessions: [] }, { status: 200 });
  }
}
