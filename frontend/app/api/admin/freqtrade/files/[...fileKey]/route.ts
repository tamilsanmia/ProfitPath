...
export async function GET(
  ...
  try {
    const res = await fetchBackend(`/admin/freqtrade/files/${key}`, { cache: "no-store" });
...
export async function PUT(
  ...
  try {
    ...
    const res = await fetchBackend(`/admin/freqtrade/files/${key}`, {
      method: "PUT",
...
export async function DELETE(
  ...
  try {
    const res = await fetchBackend(`/admin/freqtrade/files/${key}`, { method: "DELETE" });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch {
