...
export async function GET() {
  ...
  try {
    const res = await fetchBackend("/admin/freqtrade/ssh-key", { cache: "no-store" });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch {
    return NextResponse.json({ error: "Failed to check SSH key" }, { status: 502 });
  }
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetchBackend("/admin/freqtrade/ssh-key", {
      method: "PUT",
    ...
    return NextResponse.json({ error: "Failed to upload SSH key" }, { status: 502 });
  }
}
